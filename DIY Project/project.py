...
haar_hand = cv.CascadeClassifier('hand.xml')
...
with map_face_mesh.FaceMesh(min_detection_confidence =0.5, min_tracking_confidence=0.5) as face_mesh:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            if train_count == 1:
                train_count = 2
                user_ratio = user_ratio/200
                sleep_ratio = user_ratio
                print(sleep_ratio)
                while True:
                    blank_frame = np. zeros(shape=[512,712,3],dtype=np. uint8)
                    cv.putText(blank_frame, f'Ratio(User): {sleep_ratio}', (60, 180), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), thickness=2)
